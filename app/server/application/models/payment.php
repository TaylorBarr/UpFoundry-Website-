...
class Payment extends Model
{
    ...
    function save($ignore_current_user = false)
    {
        ...
        $payment_method = $current_user == false || $current_user->role != 'admin' ? get_config('payments.method') : 'manual';
        ...
        if (!$this->is_valid_payment($invoice, $ignore_current_user)) {
        ...
        if(!isset($this->user_id) && $current_user !== false)
    ...
    function complete_payment($invoice, $payment_method, $user_id = null){
        ...
        ActivityManager::payment_created($this, $invoice, $payment_method, $user_id);
    ...
    function send_payment_notification($invoice, $payment_method, $user_id = null){
        ...
        $email->send_payment_notifications($params, $invoice->client_id);
    }
    ...
    function is_valid_payment($invoice, $ignore_current_user = false){
        if((float)$invoice->total != (float)$this->amount)
            return false;

        //this function used to check that the logged in user belonged to the client that this invoice is associated with
        //since payments are now open/public funcitonality, the check is no longer relevant
