...
class PaypalPayment extends Model
{
    ...
    public function generate_submission_button($invoice)
    {
        ...
        $ipn_handler_url = $base_url . "server/index.php?url=paypal/ipn_listener";
    ...
    function record_successful_payment()
    {
        ...
        //convert the date from the paypal format to a timestamp
        $date = new DateTime($payment->payment_date);
        $payment->set('payment_date', $date->getTimestamp());
        ...
        try{
            ...
            Log::error('Error Recording Paypal Payment', array(
    ...
    generate_ipn_status_log($success)
    {
        ...
        if ($success)
            $this->ipn_status = "SUCCESS: IPN VERIFIED!\n";
        else
            $this->ipn_status = "FAIL: IPN Verification Failed!\n";
        ...
        if ((bool)get_config('payments.paypal.log_ipn_results') == true) {
            ...
            $status = 'IPN ' . $success ? ' VERIFIED' : 'FAILURE';

            Log::error($this->ipn_status);

            $email = new AppEmail();
            $email->send_ipn_status($status, $this->ipn_data);
