...
class InvoiceStyle
{
    ...
    public $logo_width;
    public $logo_height;
    public $company_info_height;

    const MARGIN = 20;
    const INFO_CELL_HEIGHT = 5;
    const COMPANY_INFO_CELL_HEIGHT = 4;
    ...
    {
        $status = isset($data['statusText']) ? $data['statusText'] : $data['status_text'];

        $this->status = new InvoiceStatus($status);

        $this->logo = $data['company']['logo'];

        $this->company_info_height = 0;
...
class InvoiceStatus
{
    ...
    {
        switch ($status) {
            case InvoicePDF::utf8(Language::get('invoice.status_pending')):
            case InvoicePDF::utf8(Language::get('invoice.status_inactive')):
            case InvoicePDF::utf8(Language::get('estimate.status_draft')):
            case InvoicePDF::utf8(Language::get('estimate.status_sent')):
                $source = self::$Status_Inactive;
                break;

            case InvoicePDF::utf8(Language::get('invoice.status_paid')):
            case InvoicePDF::utf8(Language::get('estimate.status_approved')):
                $source = self::$Status_Paid;
                break;

            case InvoicePDF::utf8(Language::get('invoice.status_overdue')):
            case InvoicePDF::utf8(Language::get('estimate.status_rejected')):
...
class InvoicePDF extends FPDF
{
    ...
    static function utf8($data)
    {
        if (is_object($data) || is_array($data)) {
            foreach ($data as &$value)
                $value = InvoicePDF::utf8($value);
    ...
    private function InvoiceSummary()
    {
        ...
        parent::Cell(25, $height, $spacer . Language::get('invoiceView.subtotal'), 'B', 2, 'L');

        foreach($this->InvoiceData['taxes'] as $tax){
            parent::Cell(25, $height, $spacer . $tax['name'] . " (" . $tax['formattedTaxRate'] . ")", 'B', 2, 'L');
        }
       // parent::Cell(25, $height, $spacer . Language::get('invoiceView.tax') . " ($tax)", 'B', 2, 'L');
        parent::Cell(25, $height, $spacer . Language::get('invoiceView.total'), 'B', 2, 'L');
        ...
        foreach ($this->InvoiceData['taxes'] as $tax) {
            ...
            parent::Cell($width - 25, $height, $tax['formattedTaxAmount'] . $spacer, 'B', 2, 'R');
        }
    ...
    private function InvoiceItems()
    {
        ...
        $items = isset($this->InvoiceData['invoiceItems'])? $this->InvoiceData['invoiceItems'] : array();
        $row0 = true;
    ...
    public function Footer()
    {
        ...
        $this->Cell(0, 10, Language::get('invoiceView.page') . ' ' . $this->PageNo() . ' / {nb}', 0, 0, 'C');
    }
    ...
    private function InvoiceInfo()
    {
        ...
        $y1 = $margin * 2 + Units::Px_to_mm($this->style->logo_height) + $this->style->company_info_height;
        ...
        parent::SetY($y1);
        parent::SetX($margin);
        parent::Cell(10, $cell_height, Language::get('invoiceView.to') . ":");
        ...
        //due date
        parent::SetFont('Arial', 'B');
        parent::Cell($w2, $cell_height + 6, Language::get('invoiceView.due') . ':', 0, 2, 'R');
        parent::SetFont('Arial', '');
        parent::SetY($y1 + $margin);
...
//		parent::SetDrawColor(InvoiceStyle::ITEM_BORDER_R, InvoiceStyle::ITEM_BORDER_G, InvoiceStyle::ITEM_BORDER_B);
    ...
    private function InvoiceLogo()
    {
        ...
        //draw the company info

        $address1 = $this->InvoiceData['company']['address1'];
        $address2 = $this->InvoiceData['company']['address2'];
        $phone = $this->InvoiceData['company']['phone'];
        if(!$this->has_data($address1) && !$this->has_data($address2) && !$this->has_data($phone)){
            return;
        }

        $y1 = $margin + Units::Px_to_mm($this->style->logo_height);

        parent::SetFont('Arial');
        $this->SetFontPixelSize(10);
        parent::SetDrawColor(InvoiceStyle::ITEM_TH_BORDER_R, InvoiceStyle::ITEM_TH_BORDER_G, InvoiceStyle::ITEM_TH_BORDER_B);
        parent::SetY($y1);
        parent::SetX($margin);

        $cell_height = InvoiceStyle::COMPANY_INFO_CELL_HEIGHT;
        $w1 = Units::Px_to_mm(InvoiceStyle::INFO_CELL1_WIDTH);

        if($this->has_data($address1)){
            $this->style->company_info_height += $cell_height;
            parent::Cell($w1, $cell_height, $address1, 0, 2);
        }

        if($this->has_data($address2)){
            $this->style->company_info_height += $cell_height;
            parent::Cell($w1, $cell_height, $address2, 0, 2);
        }

        if($this->has_data($phone)){
            $this->style->company_info_height += $cell_height;
            parent::Cell($w1, $cell_height, $phone, 0, 0);
        }
