...
Class Message extends Model {
    ...
    function save() {
        ...
        else $user = new User($this->user_id);
        ...
        $email_params = array_merge($this->to_array(), array(
            'linked_object_title' => $this->linked_object_title,
            'linked_object_url' => Request::get('message_url'),
            'linked_object_type' => strtolower(get_class($this->linked_object)),
            'posted_by' => $user->first_name,
            'posted_by_user_id' => $user->id,
            'reply_to_extension' => $this->reference_object . '-' . $this->reference_id
        ));

        $email->send_message_notification($email_params, $reference_object);
    ...
    function name_field($object) {
        if ($object == 'invoice' || $object == 'estimate')
            $name = 'number';
        else if ($object == 'task')
            $name = 'task';
        else $name = 'name';
    ...
    function remove_html_tags_from_messages(){
        $sql = "SELECT id, message FROM messages";
        $messages = $this->select($sql);

        foreach($messages as $message){
            $clean_message = strip_tags($message->message);
            $sql = "UPDATE messages SET message = '$clean_message' WHERE id = $message->id";
            $this->execute($sql);
        }
    ...
    function remove_special_characters_from_messages(){
        $sql = "SELECT id, message FROM messages";
        $messages = $this->select($sql);

        foreach($messages as $message){
            $clean_message = str_replace('Â', '', $message->message);
            $sql = "UPDATE messages SET message = '$clean_message' WHERE id = $message->id";
            $this->execute($sql);
        }
