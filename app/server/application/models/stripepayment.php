...
class StripePayment extends Model
{
    ...
    function init($amount, $post_params, $invoice = null)
    {
        ...
        if (!isset($_POST['stripeToken'])) {
            $this->set_error('amount', Language::get('errors.stripe_invalid_token'));
            return false;
        }
    ...
    function submit()
    {
        try {
            ...
            $result = Stripe_Charge::create(array(
                    ...
                    "currency" => get_config('payments.stripe.currency_code'),
                    "card" => $this->token,
            ...
            if (!($result instanceof Stripe_Charge)) {
                $this->set_error('amount', Language::get('errors.stripe_error_charging_card'));
                return false;
            ...
            $this->set_error('amount', Language::get('errors.stripe_invalid_request'));
            return false;

        } catch (Stripe_ApiConnectionError $e) {
            // Network communication with Stripe failed
            $this->set_error('amount', Language::get('errors.stripe_unable_to_connect'));
            return false;
            ...
            $this->set_error('amount', Language::get('errors.stripe_general'));
            return false;
