...
class luisaImport {
	...
	public $error	= '';
	public $urls	= array(
		'luisa'		=> 'http://demo.stylishthemes.co/imbt/',
	);
	...
	function init() {
		...
		wp_enqueue_style( 'imbt.import', get_template_directory_uri(). '/admin/importer/import.css', false, time(), 'all');
		wp_enqueue_script( 'imbt.import', get_template_directory_uri() . '/admin/importer/import.js', false, time(), true );
	 ...
	 * Import | Content
	 * ---------------------------------------------------------------------------- */
	function import_content( $file = 'imbt.xml.gz' ){
		$import = new WP_Import();
		$xml = get_template_directory() . '/admin/importer/demo/'. $file;
	...
	function import_menu_location( $file = 'menu.txt' ){
		$file_path 	= get_template_directory_uri() . '/admin/importer/demo/'. $file;
...
// 				print_r($_POST);
				...
				if( ! class_exists( 'WP_Import' ) ){
					require_once get_template_directory() . '/admin/importer/wordpress-importer.php';
				}
				...
				if( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ){
					...
					switch( $_POST['import'] ) {
						...
						default:
							// Empty select.import
							$this->error = __('Please select data to import.', 'stylish');
							break;
					}
					...
					if( $this->error ){
						...
						echo '<div class="updated settings-error">';
							echo '<p><strong>'. __('All done. Have fun!', 'stylish') .'</strong></p>';
						echo '</div>';
					}
    ...
    function create_menu_code(){
        ...
        $data = array('main-menu' => 'Main Menu', 'social' => 'Social');
